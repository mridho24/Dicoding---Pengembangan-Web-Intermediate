...
class MapHelper {
  ...
  addStoryMarkers(stories) {
    ...
    storiesWithLocation.forEach((story) => {
      const marker = L.marker([story.lat, story.lon], { icon: this._markerIcon });
  ...
  setupLocationSelector(onLocationSelected) {
    ...
    this._map.on("click", (event) => {
      ...
      const marker = L.marker([lat, lng], { icon: this._markerIcon }).addTo(this._map);
      this._markers.push(marker);
      ...
      navigator.geolocation.getCurrentPosition(
        (position) => {
          ...
          if (this._map) {
            ...
            this.clearMarkers();
            const marker = L.marker([location.lat, location.lon], { icon: this._markerIcon })
              .addTo(this._map)
